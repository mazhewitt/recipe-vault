...
The Frontend XSS Protection capability provides HTML sanitization and XSS prevention for user-provided content in the browser.
...
### Security
- All user-provided data MUST be sanitized before DOM insertion unless proven safe (e.g., markdown from trusted LLM responses with controlled rendering)
- Code reviews for frontend changes SHOULD include XSS security checks
- Automated testing SHOULD include XSS payload test cases

### Performance
- `escapeHtml()` function SHOULD execute in under 1ms for typical recipe text (< 10KB)
- Sanitization overhead SHOULD not noticeably impact rendering performance
